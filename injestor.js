...
const dataBuilder = require("./injestor/modules/dataBuilder.js");
...
async function run() {
  try {
    ...
    const taxDeedDB = database.collection(db);

    const pdfFilePath = "./injestor/files/2024-August.PDF";
    const pageNumber = 10; // Change this to the desired page number
    extractTextFromPDF(pdfFilePath, pageNumber)
      .then((text) => {
        ...
        const pagePromises = pages.map(async (page, index) => {
          ...
          const dataUrls = parcelIds.map((parcelId) => {
            const url =
              "https://propertyappraisal.dekalbcountyga.gov/datalets/datalet.aspx?mode=profileall&UseSearch=no&pin=" +
              parcelId;
            return axios
              .get(url)
              .then((response) => {
          ...
          const valueUrls = parcelIds.map((parcelId) => {
            const url =
              "https://publicaccess.dekalbtax.org/datalets/datalet.aspx?mode=value_history&UseSearch=no&pin=" +
              parcelId;
            return axios
              .get(url)
              .then((response) => {
          ...
          const taxUrls = parcelIds.map((parcelId) => {
            const url =
              "https://publicaccess.dekalbtax.org/datalets/datalet.aspx?mode=dek_profile&UseSearch=no&pin=" +
              parcelId;
            return axios
              .get(url)
              .then((response) => {
                ...
                const penalties = dataBuilder.buildTaxPenalties(
                  parcelId,
                  url,
                  $
                );
          ...
          const payoffUrls = parcelIds.map((parcelId) => {
            const url =
              "https://publicaccess.dekalbtax.org/datalets/datalet.aspx?mode=dek_prof_c10&UseSearch=no&pin=" +
              parcelId;
            return axios
              .get(url)
              .then((response) => {
        ...
        Promise.all(pagePromises)
          .then(async (results) => {
            ...
            var properties = Object.entries(resultsByParcelId).map(
              ([key, value]) => ({
                parcelId: key,
                ...value,
              })
